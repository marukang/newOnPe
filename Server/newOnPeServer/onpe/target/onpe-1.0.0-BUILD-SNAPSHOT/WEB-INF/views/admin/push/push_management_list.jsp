...
<html>
					...
					<div class="box-content">
						<div class="overflow">
							<table class="basic medium">
								...
								<c:forEach items="${push_list}" var="push_list">
									<tr class="push_list_tr" onclick="location.href='/admin/push/push_management_detail?mode=modify&push_number=${push_list.push_number}'">
										<td><c:out value="${sum}"/></td>
										<td>${push_list.push_title}</td>
										<td>${fn:substring(push_list.push_create_date,0,4)}-${fn:substring(push_list.push_create_date,4,6)}-${fn:substring(push_list.push_create_date,6,8)}</td>
										<td>${fn:substring(push_list.push_reservation_time,0,4)}-${fn:substring(push_list.push_reservation_time,4,6)}-${fn:substring(push_list.push_reservation_time,6,8)} ${fn:substring(push_list.push_reservation_time,8,10)}:${fn:substring(push_list.push_reservation_time,10,12)}</td>
									</tr>
						...
						<div class="btn-wrap">
							<button class="btn-pt" onclick="location.href='/admin/push/push_management_detail?mode=create'">푸시 알림 등록</button>
						</div>
	...
	$(document).ready(function(){
		...
		function form_check(){
			...
			var url = "/admin/push/push_management_list?ck=1";
			if(option != "전체"){
				url += "&option="+option;
			}
			if(keyword.length != 0){
