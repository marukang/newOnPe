...
<html>
	...
	<div class="wrap">
		...
		<div class="content">
			<div class="wrapper">
				<h2 class="sub-title">관리자 메뉴 - 1:1문의 관리</h2>
						...
						<div class="filter-inner flex">
							<div class="field w100">
								...
								<select id="option_selectBox">
									<c:if test="${option == null or option.equals('전체')}">
									<option selected="selected">전체</option>
									<option>아이디</option>
									<option>이름</option>
									</c:if>
									<c:if test="${option == '아이디'}">
									<option>전체</option>
						...
						<div class="overflow">
							<table class="basic medium">
								<tr>
									...
									<th style="width:32%;">문의제목</th>
									<th style="width:10%;">작성일</th>
									<th style="width:8%;">답변상태</th>
								</tr>
								<c:set var="sum" value="${(page-1)*15 + 1}" />
								<c:forEach items="${qna_list}" var="qna_list">
									<tr class="qna_list_tr" onclick="location.href='/admin/etc/qna_detail?mode=modify&question_number=${qna_list.question_number}'">
										<td style="width:5%;"><c:out value="${sum}"/></td>
										<td style="width:15%;">${qna_list.question_id}</td>
										<td style="width:15%;">${qna_list.question_name}</td>
										<td style="width:15%;">${fn:substring(qna_list.question_phonenumber,0,3)}-${fn:substring(qna_list.question_phonenumber,3,7)}-${fn:substring(qna_list.question_phonenumber,7,11)}</td>
										<td style="width:32%;">${qna_list.question_title}</td>
										...
										<c:if test="${qna_list.question_state == '0'}">
											<td style="width:8%;">미답변</td>
										</c:if>
										<c:if test="${qna_list.question_state == '1'}">
											<td style="width:8%;">답변 완료</td>
										</c:if>
									</tr>
	...
	<script>
	var global_keyword = "${keyword}";
	var global_option = "${option}";
	$(document).ready(function(){
		
		/* 상단 초기화 버튼 클릭 이벤트 */
		$("#reset_btn").click(function(){
			location.href="/admin/etc/qna_list"
		});
		...
		function form_check(){
			...
			if(option != "전체" && option != "아이디" && option != "이름"){
			...
			var url = "/admin/etc/qna_list?ck=1";
