...
<html>
	...
	<div class="wrap">
		...
		<div class="content">
			<div class="wrapper">
				<h2 class="sub-title">관리자메뉴 - FAQ 관리</h2>
				...
				<div class="box-wrap">
					<div class="box-content">						
							...
							<div class="form auto-w">
								<div class="field w69">
									<div class="field-inner">
										<label>제목</label>
										<input id="faq_title" type="text" class="w69" placeholder="FAQ 제목을 입력해주세요" value="${faq.faq_title}">
									</div>
								</div>
								...
								<div class="field w24">
									<div class="field-inner">
										<label>타입</label>
										<input id="faq_type" type="text" placeholder="FAQ 타입을 입력해주세요" value="${faq.faq_type}">
									</div>
								</div>
								...
								<div class="field w100">
									<div class="field-inner">
										<label>내용</label>
										<textarea id="faq_content" placeholder="FAQ 내용을 입력해주세요">${faq.faq_content}</textarea>
									</div>
								</div>
							</div>
						</div>
						...
						<div class="btn-wrap">
							...
							<button class="btn-sec" onclick="location.href='/admin/etc/faq_list'">목록</button>
						</div>
						</c:if>
	...
	$(document).ready(function(){
		...
		$("#modify_btn").click(function(){
			...
			if(faq_title.length < 2 || faq_title.length > 80){
				alert("제목을 2 ~ 80자로 입력해주세요");
				return;
			}
			if(faq_content.length < 5 || faq_content.length > 3000){
				alert("내용을 5 ~ 3000자로 입력해주세요");
				return;
			}
			if(faq_type.length < 2 || faq_type.length > 20){
				alert("타입을 2 ~ 20자로 입력해주세요");
				return;
			}
			...
			$.ajax({
				type:"POST",
				url:"/admin/etc/faq_detail_work",
				data:data,
				dataType:"text",
				success:function(string){
					if(string == "fail"){
						alert("FAQ 수정에 실패했습니다.\r다시 시도해주세요");
					}else{
						alert("FAQ를 수정했습니다.");
						history.go(0);
					}
		...
		$("#create_btn").click(function(){
			...
			$.ajax({
				...
				success:function(string){
					if(string == "fail"){
						alert("FAQ 등록에 실패했습니다.\r다시 시도해주세요");
					}else{
						alert("fAQ를 등록했습니다.");
