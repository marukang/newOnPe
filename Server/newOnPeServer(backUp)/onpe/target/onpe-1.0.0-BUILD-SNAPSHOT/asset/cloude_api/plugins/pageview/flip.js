...
eBookCore.pageTurn.init = function(){
	...
	// 확대용 스킨이면 확대 가능하게 설정 jr  
	if (eBookData.pageView.type == 'flip'){
		if(eBookData.pageView.pageZoomMax != undefined){
		    var  zoomMax = eBookData.pageView.pageZoomMax;
			if (zoomMax< 101)
			{ zoomMax = 100;
			};
		}
		...
		if(eBookData.pageView.pageZoomMin){
	    var  zoomMin = eBookData.pageView.pageZoomMin;
		}
		else { var  zoomMin = eBookSkin.options.pageZoomMin; }
		if(zoomMin <100) { zoomMin= 100 };   // 축소비율이 100% 이하로 작아지지않게 
		zoomMin = zoomMin/100;


		/* 2018-06-11 이미지 확대 jr */
		   $('#scale').slider({
...
// 모바일 핀치 확대 2018-07-17 jr  (핀치 재수정 2019-05-13)
...
//	.on(eBookCore.eventType.click,function(){ /** 클릭시 다음 화면 -- 모바일에서 페이지 두장씩 넘어가는 오류로 삭제 jr 20201022 */
//			eFnc.gotoNext();
			// 페이지 사이즈 조정
//			eBookCore.pageTurn.resize();
//			eBookCore.plugins.setPagePos();
...
/** 2018-06-11 페이지 위치를 화면 안으로 강제 시킴 추가 zoomview 에서 가져옴 jr */
...
/** 휠 확대축소 이벤트 처리 함수   2018-07-11 jr zoomview에서 가져옴 */
