...
<html>
					...
					<div class="box-content">
						<div class="form">
							<div class="field w32">
								<div class="field-inner">
									<label>이름</label>
									<input id="teacher_name" type="text" placeholder="선생님 이름을 입력해주세요" value="${teacher_information.teacher_name}">
								</div>
							</div>
							<div class="field w32">
								<div class="field-inner">
									<label>학교</label>
									<input id="teacher_school" type="text" value="${teacher_information.teacher_school}" class="input-m">
							...
							<div class="field w32">
								<div class="field-inner">
									<label>생년월일</label>
									<input id="teacher_birth" type="number" placeholder="ex) 19900510" value="${teacher_information.teacher_birth}">
								</div>
							</div>
							<div class="field w32">
								<div class="field-inner">
									...
									<c:if test="${mode == 'modify'}">
										<c:if test="${teacher_information.teacher_sex eq 'm'}">
											<option selected>남자</option>
											<option>여자</option>
										</c:if>
										<c:if test="${teacher_information.teacher_sex eq 'f'}">
											<option>남자</option>
							...
							<div class="field w32">
								<div class="field-inner">
									<label>아이디</label>
									<c:if test="${mode == 'modify'}">
										<input id="teacher_id" type="text" value="${teacher_information.teacher_id}" disabled>
									</c:if>
									<c:if test="${mode == 'create'}">
										<input id="teacher_id" type="text" value="${teacher_information.teacher_id}" placeholder="아이디를 입력해 주세요">
									</c:if>
								</div>
							</div>
							...
							<c:if test="${mode == 'modify'}">
								<div class="field w32">
									<div class="field-inner">
										<label>비밀번호</label>
										<button id="teacher_password_reset" class="btn-s-round bg-blue">초기화</button>
									</div>
								</div>
							</c:if>
							<c:if test="${mode == 'create'}">
							...
							<div class="field w49">
								<label>E-mail</label>
								<input id="teacher_email" type="email" placeholder="이메일 주소를 입력하세요" value="${teacher_information.teacher_email}">
								<label>
								<c:if test="${teacher_information.teacher_email_agreement == '1'}">
							...
							<div class="field w49">
								<label>선생님 휴대폰</label>
								<input id="teacher_phone" type="number" placeholder="휴대폰 번호를 입력하세요." value="${teacher_information.teacher_phone}">
								<label>
								<c:if test="${teacher_information.teacher_message_agreement == '1'}">
						...
						<c:if test="${mode == 'modify'}">
							<c:if test="${teacher_information.teacher_recent_join_date != null }">
								<p class="txt-right txt-small">최근 접속일: ${fn:substring(teacher_information.teacher_recent_join_date,0,4)}-${fn:substring(teacher_information.teacher_recent_join_date,4,6)}-${fn:substring(teacher_information.teacher_recent_join_date,6,8)}</p>
							</c:if>
							<c:if test="${teacher_information.teacher_recent_join_date == null }">
	...
	<script>		
		$(document).ready(function(){
			...
			/* 비밀번호 초기화 버튼 클릭 이벤트 */
			$("#teacher_password_reset").click(function(){
			...
			$("#school_choose").click(function(){
				if($("#school_list_parent").children('.active').size() == 1){
					...
					$("#teacher_school").val($("#school_list_parent").children('.active').text());
					$("#search_school_popup").css("display","none");
			...
			// 등록하기 버튼 클릭 이벤트
			// 반환값으로 중복아이디, 중복메일, 중복핸드폰번호, 등록 성공 유무 반환 
			$("#create_btn").click(function(){
				...
				if(teacher_name == null || teacher_name.length < 1 || teacher_name.length > 10){
				...
				}else if(teacher_birth == null || teacher_birth.length != 8){
					alert("생년월일을 8자리 숫자로 입력해주세요");
				}else if(teacher_sex != "남자" && teacher_sex != "여자"){
					alert("성별을 확인해 주세요");
					$("#teacher_sex").focus();
					...
					$.ajax({
						type:"POST",
						url:"/admin/member//lms_member_management_detail_work",
						data:params,
						success:function(string){
							if(string == "fail"){
								...
								alert("회원 등록에 성공했습니다.");
								location.href="/admin/member/lms_member_management_detail?teacher_id="+teacher_id+"&mode=modify";
							}
						},
			...
			// 수정하기 버튼 클릭 이벤트
			// 반환값으로 중복메일, 중복핸드폰번호, 수정 성공 유무 반환 
					...
					$.ajax({
						...
						success:function(string){
							if(string == "success"){
								...
								location.href="/admin/member/lms_member_management_list"
							}else{
