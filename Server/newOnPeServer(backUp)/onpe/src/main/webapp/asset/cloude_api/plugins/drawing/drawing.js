...
eBookCore.plugins.drawing.run = function(_currentPageNum) {
    ...
    if (0 < $("#ebookdrawing").length) {
        return $("#ebookdrawing").detach();
    }
    ...
    toolsEl.css({
        left: (window.innerWidth - toolsEl.outerWidth()) * 0.5,
    ...
    if (!croTools.isMobile()) {
        $("a.drawingColors").each(function(i, e) {
            $("<span>" + eBookCore.plugins.drawing.colorKeys[i] + "</span>")
                .appendTo(e).on(eBookCore.eventType.click, function() {
    ...
    drawingMenuEl.append($('<img class="drawingBtns close" src="' + eBookCore.resource.close + '" title="' + eBookCore.getString('close') + '(ESC)"/>').on(eBookCore.eventType.click, function() {
        ...
        if (0 < _actions.length) {
            eBookCore.plugins.drawing.drawingData[_currentPageNum] = JSON.stringify(_actions);
            _drawingData[_currentPageNum] = JSON.stringify(_actions);
        } else {
            eBookCore.plugins.drawing.drawingData[_currentPageNum] = null;
            _drawingData[_currentPageNum] = null;
        }
        _drawingData = eBookCore.plugins.drawing.drawingData;
        console.log(JSON.stringify(eBookCore.plugins.drawing.drawingData));
        console.log(JSON.stringify(_drawingData));

        // 2017.01.21 김대원	cookie에 저장
        //		$.cookie( _cookieHeader, _drawingData, { expires : 365*20, path : '/' } );
        //		window.localStorage.setItem('ebookdrawing', JSON.stringify(_drawingData));
        if (typeof(localStorage) != 'undefined') {
            localStorage[_cookieHeader] = JSON.stringify(_drawingData);
        }
        // 드로잉 등록 이벤트 제거
        $(window).off(".drawing");
    ...
    toolsEl.draggable({
        opacity: 0.9,
        cancel: ".drawingBtns, .drawingColors, .drawingSizes"
    });

    // 각 버튼들 드래그 이벤트 방지
    $(".drawingColors, .drawingSizes, .drawingBtns").on('dragstart', function(e) {
        e.preventDefault();
    });
    ...
    //	var _drawingData = $.cookie(_cookieHeader);
    if (typeof(localStorage) != 'undefined') {
        var _localDrawData = localStorage.getItem(_cookieHeader);
        var _drawingData = {};

        if (_localDrawData) {
            _drawingData = JSON.parse(_localDrawData);
            console.log("_drawingData >> " + JSON.stringify(_drawingData[_currentPageNum]));
        }
        //	var _drawingData = JSON.parse($.localStorage[_cookieHeader]);
        //	console.log ("eBookCore.plugins.drawing.drawingData >> " + eBookCore.plugins.drawing.drawingData[_currentPageNum]);
        // 2017.01.23 김대원
        if (_drawingData[_currentPageNum]) {
            var _tmpDrawData = JSON.parse(_drawingData[_currentPageNum]);
            console.log("AAAAAA" + JSON.stringify(_drawingData[_currentPageNum]));
            console.log("BBBBBB" + JSON.stringify(_tmpDrawData));
            eBookCore.plugins.drawing.drawingData[_currentPageNum] = JSON.stringify(_tmpDrawData);
            //		cvEl.sketch().actions = JSON.parse(_drawingData[_currentPageNum]);
            //		cvEl.sketch().redraw();
            //		eBookCore.plugins.drawing.drawingData = drawingData.slice();
    ...
    eBookCore.plugins.drawing.redos = {
        count: (actionsObj ? actionsObj.length : 0),
        data: []
    };
