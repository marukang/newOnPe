...
import com.funidea.newonpe.my_page.agreement.personal_information_agreement_Activity
import com.funidea.newonpe.my_page.agreement.terms_of_user_information_agreement_Activity
...
class join_page_Activity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        //개인정보 이용 동의
        join_page_personal_info_agreement_textview.setOnClickListener(personal_info_agree)
        //온체육 서비스 이용 약관 동의
        join_page_terms_of_user_info_agree.setOnClickListener(terms_of_user_info_agree)
        ...
        //비밀번호 eidtText 포커스
        join_page_input_pw_edittext.onFocusChangeListener = OnFocusChangeListener { view, gainFocus ->
            if (gainFocus) {
                if (join_page_input_pw_confirm_edittext.getText().toString() != input_pw && join_page_input_pw_confirm_edittext.getText().toString() != "") {
                ...
                } else if (join_page_input_pw_confirm_edittext.getText().toString() == input_pw && join_page_input_pw_confirm_edittext.getText().toString() != "") {
        ...
        join_page_input_pw_confirm_edittext.onFocusChangeListener = OnFocusChangeListener { view, gainFocus ->
            if (gainFocus) {
                if (join_page_input_pw_confirm_edittext.getText().toString() != input_pw || input_pw == "") {
                    join_page_input_pw_confirm_linearlayout.setBackgroundResource(R.drawable.view_red_color_round_edge)
                    join_page_input_pw_confirm_textview.setTextColor(Color.parseColor("#FF0000"))
                    pw_double_check = false  }
            } else {

                if (join_page_input_pw_confirm_edittext.getText().toString() == input_pw) {
                    join_page_input_pw_confirm_linearlayout.setBackgroundResource(R.drawable.view_main_color_2_round_button)
                    join_page_input_pw_confirm_textview.setTextColor(Color.parseColor("#3378fd"))
                    pw_double_check = true
                }
        ...
        serverConnection!!.id_overlap_check(student_id).enqueue(object : Callback<ResponseBody>
        {
            ...
            override fun onResponse(call: Call<ResponseBody>, response: Response<ResponseBody>) {
                ...
                try {
                    ...
                    {
                        //서버로 중복 여부 확인해주기
                        Toast.makeText(this@join_page_Activity,"사용 가능한 아이디입니다.", Toast.LENGTH_SHORT).show()
                        join_page_check_id_textview.setTextColor(Color.parseColor("#3378fd"))
                        id_confirm_check = true
                    }
                    else if(result_value.equals("y"))
                    {
                        Toast.makeText(this@join_page_Activity,"이미 사용 중이거나, 사용이 불가능한 아이디입니다.", Toast.LENGTH_SHORT).show()
            ...
            override fun onResponse(call: Call<ResponseBody>, response: Response<ResponseBody>) {
                ...
                try {
                    ...
                    if(i.next().equals("success"))
                    {
                        ...
                        Toast.makeText(this@join_page_Activity,"중복 확인이 완료되었습니다. 해당 이메일에서 인증코드를 확인 후 입력 해주세요.", Toast.LENGTH_SHORT).show()
                    ...
                    else
                    {
                        ...
                        {
                            Toast.makeText(this@join_page_Activity,"이미 사용 중인 이메일입니다.", Toast.LENGTH_SHORT).show()
                        }
                        else
                        {
                            Toast.makeText(this@join_page_Activity,"인터넷 상태를 다시 확인해주세요.", Toast.LENGTH_SHORT).show()
            ...
            override fun onResponse(call: Call<ResponseBody>, response: Response<ResponseBody>)
            {
                try {
                    ...
                    if(i.next().equals("success")) {
                        ...
                        val intent = Intent(this@join_page_Activity, join_complete_Activity::class.java)
                        startActivity(intent)
                        finish()
                        overridePendingTransition(R.anim.anim_slide_in_right, R.anim.anim_slide_out_left)
                    }
                    ...
                    {
                        custom_toast(this@join_page_Activity, "인터넷 연결상태를 다시 확인해주세요.")
                    }
