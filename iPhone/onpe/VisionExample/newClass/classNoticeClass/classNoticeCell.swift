...
import UIKit
protocol classNoticeCelllDelegate {
    func classNoticeCellGesture(setPushList : pushList)
}

class classNoticeCell: UICollectionViewCell {
    ...
    var delegate : classNoticeCelllDelegate?
    var messageNumber : Int?
    
    var setPushList : pushList?{
        didSet{
            if let setPushList = setPushList {
                postingNumberLabel.text = "\(messageNumber ?? 1)"
                titleLabel.text = setPushList.notice_content
                writerLabel.text = "\(extensionClass.DateToString(date: setPushList.notice_date!, type: 0))"
    ...
    func noticeListCellGesture(_ : UITapGestureRecognizer){
        print("touch")
        if let setPushList = setPushList {
            delegate?.classNoticeCellGesture(setPushList: setPushList)
        }
