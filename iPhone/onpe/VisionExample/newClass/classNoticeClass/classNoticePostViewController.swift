//
//  classNoticePostViewController.swift
//  VisionExample
//
...
class classNoticePostViewController: UIViewController {
    ...
    var getPushList : pushList?{
        didSet{
            if let getPushList = getPushList{
                //content가 제목
                //title 이 내용
                titleTextField.text = getPushList.notice_content
                
                userLabel.text = "작성자 : \(getPushList.notice_name ?? "") ・ 작성일 \(extensionClass.DateToString(date: getPushList.notice_date!, type: 0))"
                contentTextView.text = getPushList.notice_title
...
//MARK: -일반 함수
extension classNoticePostViewController {
