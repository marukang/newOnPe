...
import UIKit
protocol totalNoticeListCellDelegate {
    func noticeListCellGesture(setPushList : pushList)
}

class totalNoticeListCell: UICollectionViewCell {
    ...
    var delegate : totalNoticeListCellDelegate?
    ...
    var setPushList : pushList?{
        didSet{
            if let setPushList = setPushList {
                titleLabel.text = setPushList.push_title
                writerLabel.text = "\(extensionClass.DateToString(date: setPushList.push_reservation_time!, type: 0))"
    ...
    func noticeListCellGesture(_ : UITapGestureRecognizer){
        print("touch")
        if let setPushList = setPushList {
            delegate?.noticeListCellGesture(setPushList: setPushList)
        }
