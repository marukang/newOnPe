 ...
 class CameraViewController: UIViewController {
    ...
    private let previewOverlayView : UIImageView = UIImageView()//화면 작게 나오는 뷰
    ...
    let endBtn = UIButton()//운동종료 하고 뒤로가기
    let titleLabel = UILabel()
    let cameratouchView = UIButton()
    ...
    var gifImageviewBool = false//viewWillLayoutSubviews 함수가 여러번 실행되기 때문에 imageview의 hide를 한번만 실행해 주기 위해 사용.
    ...
    lazy var activityIndicator : UIActivityIndicatorView = {
        ...
        activityIndicator.style = UIActivityIndicatorView.Style.medium // Start animation.
        activityIndicator.color = mainColor._3378fd
    ...
    //수업(실습)에서 다음 동작명 관련 ui 변수
    let backgroundView = UIView()//다음동작명 나올때 뒷배경 흐릿하게 하는 뷰
    let nextExerciseLabel = UILabel()//다음동작 명
    let previousExerciseBtn = UIButton()//돌아가기 버튼
    let pauseAndStartBtn = UIButton()//일시정지 또는 시작버튼
    let mTimer2Label = UILabel()
    var mTimer2 = Timer()//다음 동작명 나올때 실행되는 타이머
    var number2 = 0
    var nextExerciseBool = true
    //------------------------------------------------------------------------------
    ...
    var number4 = 0 //운동하는 전체 시간 저장
    
    var referenceViewWidth1 : CGFloat = 0 // 밑에 참고 이미지 width
    var referenceViewWidth2 : CGFloat = 0 // 전환 버튼 width
    var skeletonViewBool : Bool = false // 해당 값이 false이면 홈트레이닝 종목을 할때이고 true이면 홈트레이닝 이외의 종목을 할때이다.
    ...
    func sizetheFitsControlByDevice(deviceType : String)
    {
        ...
        if deviceType == "iPhone 8" || deviceType == "iPhone 7" || deviceType == "iPhone 6" || deviceType == "iPhone 6S"
        {
            ...
            referenceViewWidth1 = 1.25
            referenceViewWidth2 = 0.25
            
        } else if deviceType == "iPhone 11" || deviceType == "iPhone XR"  {
            
            referenceViewWidth1 = 1.5
            referenceViewWidth2 = 0.35
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 1){
            ...
            self._gifImageview(imageview : self.gifImageview)
            self._gifImageview2(imageview : self.gifImageview2)
        }
    ...
    public func removeFrontViews(){
        ...
        cameratouchView.removeFromSuperview()
        mTimer2Label.removeFromSuperview()
        nextExerciseLabel.removeFromSuperview()
        previousExerciseBtn.removeFromSuperview()
        pauseAndStartBtn.removeFromSuperview()
    ...
    func changeGifImage(type : Bool){
        SDImageCache.shared.clearMemory()
        SDImageCache.shared.clearDisk()
        if PoseInformation.array_count + 1 < content_url_array.count{
            if type {
                self.pose_url = self.content_url_array[PoseInformation.array_count]
            } else {
                
                self.pose_url = self.content_url_array[PoseInformation.array_count + 1]
            }
        ...
        print("히야야얏2 : ",pose_url)
        print("히야야얏2 : ",PoseInformation.array_count)
        OperationQueue.main.addOperation {
            DispatchQueue.main.async {
                self.gifImageview.sd_setImage(with: URL(string: self.pose_url), placeholderImage: nil, options: .refreshCached, completed: { image,_,_,_ in
                ...
                self.gifImageview2.sd_setImage(with: URL(string: self.pose_url), placeholderImage: nil, options: .refreshCached, completed: nil)
            }
        }
        print("hellow")
    }
        ...
        if let data = exerciseContentStr?.data(using: .utf8){
            do {
                ...
                if let content_name_list = getResult.content_name_list, let content_category_list = getResult.content_cateogry_list, let content_detail_name_list = getResult.content_detail_name_list, let count_list = getResult.content_count_list, let content_time_list = getResult.content_time, let content_url_array = getResult.content_url, let contentCode = getResult.content_code {
                    ...
                    OperationQueue.main.addOperation {
                        DispatchQueue.main.async {
                            self.gifImageview.sd_setImage(with: URL(string: self.pose_url), placeholderImage: nil, options: .refreshCached, completed: {(image, error, cacheType, url) in
                                guard image != nil else {
                                    return
                                }
                                self.startTimerBool = false
                                ...
                                //extensionClass.showToast(view: self.view, message: "영상 시작", font: UIFont.NotoSansCJKkr(type: .normal, size: extensionClass.textSize3))
    ...
    private func detectPose(in image: VisionImage, width: CGFloat, height: CGFloat) {
        if let poseDetector = self.poseDetector {
            ...
            guard nextExerciseBool else {
                print("다음 운동 시작전")
                return
            }
            guard !serverBool else {
                print("운동 끝")
                return
            }
            DispatchQueue.main.sync {
                ...
                if(PoseInformation.array_count == -1){
                    ...
                    if !serverBool {
                        ...
                        gifImageview.sd_setImage(with: nil, completed: nil)
                        gifImageview2.sd_setImage(with: nil, completed: nil)
                        SDImageCache.shared.clearMemory()
                        SDImageCache.shared.clearDisk()
                        ...
                        print("--------")
                        _captureState = .end
                        mTimer.invalidate()//각 운동에 대한 운동타이머 종료
                    ...
                    if(PoseInformation.pose_count == posecount_array[PoseInformation.array_count] || self.deadlineFinishBool){
                        ...
                        */
                        if self.posename_array.count > PoseInformation.array_count{
                            ...
                            if nextExerciseBool {
                                nextExerciseBool = false//해당 값이 false면 운동을 시작하지 않는다.
                                UIView.animate(withDuration: 0.7, delay: 0, usingSpringWithDamping: 1, initialSpringVelocity: 1, options: .curveEaseOut, animations: {
                                    //print(self.posename_array[PoseInformation.array_count])
                                    
                                    self.nextExerciseLabel.text = "\(self.posename_array[PoseInformation.array_count])"
                                    self.startTimer2()
                                    
                                    self.mTimer2Label.isHidden = false
                                    self.nextExerciseLabel.isHidden = false
                                    self.previousExerciseBtn.isHidden = false
                                    self.pauseAndStartBtn.isHidden = false
                                    
                                    self.backgroundView.isHidden = false
                                    
                                    
                                    self.mTimer2Label.alpha = 1
                                    self.nextExerciseLabel.alpha = 1
                                    self.previousExerciseBtn.alpha = 1
                                    self.pauseAndStartBtn.alpha = 1
 ...
 extension CameraViewController {
    ...
    @objc
    func previousExerciseBtnAction(){
        print(PoseInformation.array_count)
        PoseInformation.array_count -= 1
        print(PoseInformation.array_count)
        UIView.animate(withDuration: 0.7, delay: 0, usingSpringWithDamping: 1, initialSpringVelocity: 1, options: .curveEaseOut, animations: {
            self.changeGifImage(type : true)
            self.exerciseCountCollectionview.reloadData()
            self.mTimer2.invalidate()
            self.mTimer2Label.alpha = 0
            self.nextExerciseLabel.alpha = 0
            self.previousExerciseBtn.alpha = 0
            self.pauseAndStartBtn.alpha = 0
            self.backgroundView.alpha = 0 //원래 0.8 나머지는 1
            
        }, completion: {_ in
            self.pauseAndStartBtn.isSelected = false
            self.pauseAndStartBtn.setTitle("일시정지", for: .normal)
            self.pauseAndStartBtn.backgroundColor = .white
            self.pauseAndStartBtn.setTitleColor(mainColor._3378fd, for: .normal)
            
            
            self.nextExerciseBool = true
            self.number2 = 0
            self.mTimer2Label.text = "5"
            self.mTimer2Label.isHidden = true
            self.nextExerciseLabel.isHidden = true
            self.previousExerciseBtn.isHidden = true
            self.pauseAndStartBtn.isHidden = true
            self.backgroundView.isHidden = true
    ...
    @objc
    func pauseAndStartBtnAction(_ sender : UIButton){
        if sender.isSelected{
            UIView.animate(withDuration: 0.7, delay: 0, usingSpringWithDamping: 1, initialSpringVelocity: 1, options: .curveEaseOut, animations: {
                ...
                sender.setTitle("일시정지", for: .normal)
            ...
            self.mTimer2.invalidate()
            sender.setTitle("시작하기", for: .normal)
    ...
    @objc
    func setUpPreviewOverlayViewGesture(gestureRecognizer: UITapGestureRecognizer){
        
        DispatchQueue.main.async {
            if !self.previewSizeBool{
                self.previewOverlayView.isHidden = true
                self.previewOverlayView2.isHidden = false
                DispatchQueue.main.asyncAfter(deadline: .now() + 0.1){
                    self.gifImageview2.isHidden = false
                }
                self.previewSizeBool = true
                
                
            } else {
                self.previewOverlayView.isHidden = false
                self.previewOverlayView2.isHidden = true
                self.gifImageview2.isHidden = true
                self.previewSizeBool = false
            }
    ...
    func startTimer2(){
        number2 = 0
        mTimer2 = Timer.scheduledTimer(timeInterval: 1, target: self, selector: #selector(timerCallback2), userInfo: nil, repeats: true)
    }
    func startTimer3(){
    ...
    func timerCallback2(){
        number2 += 1
        let second = 6 - number2
        print(recordList)
        mTimer2Label.text = "\(second)"
        if second == 0 {
 ...
 extension CameraViewController {
    ...
    func setupLayout(){
        ...
        setUpPreviewOverlayView(preview: previewOverlayView)
        ...
        if !recognitionSetupBool{
            ...
            _viewSwtichBtn(button: viewSwitchBtn)
            ...
            _cameratouchView(buttoon : cameratouchView)
            
            frontView?.addSubview(activityIndicator)
            activityIndicator.startAnimating()
            
            _backgroundView(view : backgroundView)
            _nextExerciseLabel(label : nextExerciseLabel)
            _previousExerciseBtn(button : previousExerciseBtn)
            _pauseAndStartBtn(button : pauseAndStartBtn)
            _mTimer2Label(label : mTimer2Label)
    ...
    private func setUpPreviewOverlayView(preview : UIImageView) {
        ...
        let topConstant = (view.frame.height * 0.4) * 1.05 // <- * 값이 커질수록 높이가줄어든다.
        preview.topAnchor.constraint(equalTo: cameraView.topAnchor, constant: topConstant).isActive = true
        preview.leadingAnchor.constraint(equalTo: cameraView.leadingAnchor, constant: view.frame.height * referenceViewWidth1).isActive = true
    ...
    private func _gifImageview(imageview : UIImageView){
        cameraView.addSubview(imageview)
        imageview.translatesAutoresizingMaskIntoConstraints = false
        imageview.contentMode = .scaleAspectFit
        
        //imageview.sd_setImage(with: URL(string: "https://onpe.co.kr/mobile/jumping1.gif"), completed: nil)
        NSLayoutConstraint.activate(
            [
    ...
    private func _gifImageview2(imageview : UIImageView) {
        if let view = frontView {
            ...
            if !gifImageviewBool{
                ...
                gifImageviewBool = !gifImageviewBool
            }
            imageview.layer.borderWidth = 3
            imageview.layer.borderColor = mainColor._3378fd.cgColor
            //imageview.sd_setImage(with: URL(string: "https://onpe.co.kr/mobile/jumping1.gif"), completed: nil)
            //375.0
            //667.0
            print(view.frame.height)
            print(view.frame.width)
            let topConstant = (view.frame.height * 0.4) * 1.05
            imageview.topAnchor.constraint(equalTo: view.topAnchor, constant: topConstant ).isActive = true
            imageview.leadingAnchor.constraint(equalTo: view.leadingAnchor, constant: view.frame.height * referenceViewWidth1).isActive = true
            
            imageview.trailingAnchor.constraint(equalTo: view.trailingAnchor, constant: view.frame.height * 0.07 * -1).isActive = true
            imageview.bottomAnchor.constraint(equalTo: view.bottomAnchor, constant:  view.frame.height * 0.1 * -1).isActive = true
        }
    ...
    private func _viewSwtichBtn(button : UIButton){
        if let view = frontView {
            ...
            button.alpha = 0.92
            button.titleLabel?.font = UIFont.NotoSansCJKkr(type: .normal, size: 10)
            button.setTitle("화면 전환", for: .normal)
            button.setImage(#imageLiteral(resourceName: "color1_refresh_image"), for: .normal)
            button.titleEdgeInsets.left = 10
            button.contentHorizontalAlignment = .center
            button.imageView?.contentMode = .scaleAspectFit
            button.setTitleColor(mainColor._3378fd, for: .normal)
            button.addTarget(self, action: #selector(viewSwitchBtnAction), for: .touchUpInside)
            ...
            //button.topAnchor.constraint(equalTo: frontView!.topAnchor).isActive = true
            
            button.centerXAnchor.constraint(equalTo: gifImageview2.centerXAnchor).isActive = true
            button.bottomAnchor.constraint(equalTo: gifImageview2.topAnchor).isActive = true
            button.leadingAnchor.constraint(equalTo: gifImageview2.leadingAnchor, constant: view.frame.height * referenceViewWidth2).isActive = true
            
            button.trailingAnchor.constraint(equalTo: gifImageview2.trailingAnchor,constant: 0).isActive = true
    ...
    func _retryBtn(button : UIButton){
        if let view = frontView{
            view.addSubview(button)
            button.translatesAutoresizingMaskIntoConstraints = false
            button.setTitle("동작 다시하기", for: .normal)
    ...
    func _titleLabel(label : UILabel){
        if let view = frontView {
            ...
            label.textColor = .black//mainColor.hexStringToUIColor(hex: "#ffffff")
    ...
    func _cameratouchView(buttoon : UIButton){
        if let view = frontView{
            view.addSubview(buttoon)
            buttoon.translatesAutoresizingMaskIntoConstraints = false
            buttoon.backgroundColor = .clear
            buttoon.addTarget(self, action: #selector(viewSwitchBtnAction), for: .touchUpInside)
            
            buttoon.topAnchor.constraint(equalTo: previewOverlayView.topAnchor).isActive = true
            buttoon.leadingAnchor.constraint(equalTo: previewOverlayView.leadingAnchor).isActive = true
            buttoon.trailingAnchor.constraint(equalTo: previewOverlayView.trailingAnchor).isActive = true
            buttoon.bottomAnchor.constraint(equalTo: previewOverlayView.bottomAnchor).isActive = true
    ...
    func _previousExerciseBtn(button : UIButton){
        if let view = frontView {
            view.addSubview(button)
            button.translatesAutoresizingMaskIntoConstraints = false
            button.setTitle("돌아가기\n(다시반복)", for: .normal)
            button.setTitleColor(mainColor._3378fd, for: .normal)
            ...
            button.alpha = 0
            button.addTarget(self, action: #selector(previousExerciseBtnAction), for: .touchUpInside)
            button.isHidden = true
            
            button.topAnchor.constraint(equalTo: self.nextExerciseLabel.bottomAnchor, constant: 0).isActive = true
            button.leadingAnchor.constraint(equalTo: self.view.leadingAnchor, constant: view.frame.height * 0.2).isActive = true
            button.widthAnchor.constraint(equalToConstant: view.frame.height * 0.65).isActive = true
            button.heightAnchor.constraint(equalToConstant: view.frame.height * 0.2).isActive = true
        }
    }
    func _pauseAndStartBtn(button : UIButton){
        if let view = frontView {
            view.addSubview(button)
            button.translatesAutoresizingMaskIntoConstraints = false
            ...
            button.layer.borderColor = mainColor._3378fd.cgColor
            button.backgroundColor = .white
            button.addTarget(self, action: #selector(pauseAndStartBtnAction(_:)), for: .touchUpInside)
            button.isHidden = true
    ...
    func _mTimer2Label(label :UILabel){
        if let view = frontView{
            view.addSubview(label)
            label.translatesAutoresizingMaskIntoConstraints = false
            label.textColor = .red
            label.textAlignment = .center
            label.font = UIFont.NotoSansCJKkr(type: .medium, size: 100)
            //   label.text = "1"
            label.alpha = 0
            label.isHidden = true
            
            label.centerXAnchor.constraint(equalTo: view.centerXAnchor).isActive = true
            label.topAnchor.constraint(equalTo: pauseAndStartBtn.bottomAnchor, constant: 0).isActive = true
