...
class idFindViewController: UIViewController {
    ...
    var findIdBool = false
    let find_id_btn = UIButton()
    
    let AF = ServerConnectionLegacy()

}
extension idFindViewController : appidFindDelegate{
    func appIdFind(result: Int, id: String) {
        if id != "" {
            
            let vc = idFindResultViewController()
            vc.label2.text = "회원님의 아이디는 \(id)입니다."
...
extension idFindViewController : UITextFieldDelegate{
...
//MARK: - 일반 함수
extension idFindViewController
{
    ...
    func checkAllRequireComplete(){
        if nameBool, emailBool {
            ...
            find_id_btn.backgroundColor = mainColor._3378fd
        } else {
            findIdBool = false
            find_id_btn.backgroundColor = mainColor._9f9f9f
    ...
    @objc
    func findidButtonAction(_ sender : UIButton)
    {
        
        if findIdBool {
            
            let name = name_text_field.text
            let email = email_text_field.text
            AF.appFindId(student_name: name ?? "", student_email: email ?? "", url: "app/find_id")
...
extension idFindViewController
{
    func setupLayout()
    {
        ...
        emailTextField(textField:email_text_field, textField1: name_text_field, text: "이메일을 입력해주세요.")
        setLabel(label: email_label, textField: email_text_field, text: "이메일 주소", width: 68)
        
        
        _confirmBtn(button: find_id_btn)
    ...
    func _topLabel(){
        ...
        topLabel.text = "가입 당시 입력한 이름/이메일을 입력해주세요."
        topLabel.font = UIFont.NotoSansCJKkr(type: .normal, size: extensionClass.textSize4)
    ...
    func _confirmBtn(button : UIButton){
        ...
        button.addTarget(self, action: #selector(findidButtonAction), for: .touchUpInside)
